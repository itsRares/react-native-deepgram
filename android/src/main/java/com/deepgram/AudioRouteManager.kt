 ...
 * earpiece / bluetooth / auto) and forwards system route changes (headphone
 * plug/unplug, Bluetooth connect/disconnect, communication-device switches)
 * back to the module so they can be surfaced to JS as the
 * `DeepgramRouteChange` event.
 *
 ...
 * the legacy `setSpeakerphoneOn` / `startBluetoothSco` switches. Both paths
 * take effect most reliably while the module is in a communication use case —
 * playback uses `USAGE_VOICE_COMMUNICATION` and the AEC capture path sets
 * `MODE_IN_COMMUNICATION`, so the Voice Agent (the primary consumer of route
 * control) always qualifies.
 */
internal class AudioRouteManager(
  ...
  private var communicationDeviceListener:
    AudioManager.OnCommunicationDeviceChangedListener? = null
  ...
  fun start() {
    ...
    // Device add/remove callbacks miss route flips between already-connected
    // devices (e.g. speaker -> earpiece). API 31+ surfaces those directly.
  ...
  fun stop() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
      communicationDeviceListener?.let {
        try {
          manager.removeOnCommunicationDeviceChangedListener(it)
        } catch (e: Exception) {
          Log.w(TAG, "removeOnCommunicationDeviceChangedListener failed", e)
        }
      }
    }
  ...
  /**
   * Apply a preferred route. Throws [IllegalArgumentException] for an unknown
   * route keyword and [IllegalStateException] when the platform AudioManager
   * is unavailable, so the caller can reject the promise with the matching
   * contract code.
   */
